...
# --- Google image generation (Imagen) fallback --------------------------
# Direct REST call to the Generative Language API's Imagen image-generation
# endpoint. Only engaged when an API key is configured; otherwise the worker
# keeps the deterministic static-poster path (graceful skip). No Cloud Run,
# no Docker, no SDK — a single authorized httpx POST. Returns the generated
# PNG bytes, or None on any failure (no key, network, non-2xx, malformed).
IMAGEN_API_KEY_ENV = "GOOGLE_API_KEY"        # primary (explicit Google key)
IMAGEN_ALT_KEY_ENV = "GEMINI_API_KEY"        # fallback (shared Gemini key)
IMAGEN_MODEL_ENV = "IMAGEN_MODEL"
IMAGEN_DEFAULT_MODEL = "imagen-3.0-generate-002"

def get_imagen_key() -> str:
    """Return the configured Google/Imagen API key, preferring GOOGLE_API_KEY."""
    return (os.environ.get(IMAGEN_API_KEY_ENV) or os.environ.get(IMAGEN_ALT_KEY_ENV) or "").strip()

def imagen_configured() -> bool:
    """True iff an Imagen image-generation key is present for the fallback."""
    return bool(get_imagen_key())

def generate_image_via_imagen(prompt: str, timeout: float = 60.0) -> Optional[bytes]:
    """Generate a 1080x1080 PNG from `prompt` via the Imagen REST endpoint.

    Returns raw PNG bytes on success, or None on any failure. Uses the native
    Generative Language `predict` endpoint with the image-generation payload.
    Zero-maintenance: never raises; the caller should fall back to the static
    poster when this returns None.
    """
    key = get_imagen_key()
    if not key:
        return None
    model = (os.environ.get(IMAGEN_MODEL_ENV) or IMAGEN_DEFAULT_MODEL).strip()
    url = (
        f"{DEFAULT_GEMINI_REST_ENDPOINT.rstrip('/')}/models/{model}:predict"
    )
    payload = {
...
# Direct REST call to the Generative Language API's Veo video-generation
# endpoint (`models/<veo>:generateContent` with video response). Returns a
# downloadable MP4 URL, or None on any failure (no key, async-op refused,
# network, non-2xx). Graceful skip: when unset/unsupported the worker keeps
# the deterministic FFmpeg Ken Burns path. Mirrors the Imagen helper above.
VEO_KEY_ENV = "GOOGLE_API_KEY"          # primary (shared Google key)
VEO_ALT_KEY_ENV = "GEMINI_API_KEY"      # fallback (shared Gemini key)
VEO_MODEL_ENV = "VEO_MODEL"
VEO_DEFAULT_MODEL = "veo-3.0-generate-001"

def get_veo_key() -> str:
    return (os.environ.get(VEO_KEY_ENV) or os.environ.get(VEO_ALT_KEY_ENV) or "").strip()

def veo_configured() -> bool:
    """Veo is opt-in: requires a key AND USE_GOOGLE_VEO=1.

    FFmpeg Ken Burns stays the zero-cost default; real AI video generation
    (paid + async) only fires when the operator explicitly opts in.
    """
    return bool(get_veo_key()) and os.environ.get("USE_GOOGLE_VEO", "").strip() not in ("", "0", "false", "False")

def generate_video_via_veo(prompt: str, timeout: float = 90.0) -> Optional[str]:
    """Request a short video from `prompt` via the Veo REST endpoint.

    Returns a direct MP4 URL on success, or None on any failure / if generation
    is not available in the configured region. Async Veo jobs are surfaced via
    `operation` — the worker treats that as graceful skip (keeps FFmpeg) rather
    than blocking, to stay zero-maintenance.
    """
    key = get_veo_key()
    if not key:
        return None
    model = (os.environ.get(VEO_MODEL_ENV) or VEO_DEFAULT_MODEL).strip()
    url = (
        f"{DEFAULT_GEMINI_REST_ENDPOINT.rstrip('/')}/models/{model}:generateContent"
    )
    payload = {
        "contents": [{"parts": [{"text": f"Short 4s square social video for a pest-control brand: {prompt}"}]}],
        "generationConfig": {
            "responseModalities": ["VIDEO"],
            "videoConfig": {"aspectRatio": "1:1", "durationSeconds": 4},
        },
    }
    try:
        resp = httpx.post(url, params={"key": key}, json=payload, timeout=timeout)
        resp.raise_for_status()
        data = resp.json()
        # Direct sync MP4 (byte base64 in parts) OR async operation handle.
        candidates = data.get("candidates") or []
        if candidates:
            for part in (candidates[0].get("content", {}).get("parts") or []):
                if part.get("videoMetadata") or part.get("inlineVideoData"):
                    # Veo emits an async operation handle you poll; we surface
                    # the id so an operator can pull the finished MP4.
                    return part.get("videoMetadata", {}).get("id") or "veo-pending"
        oper_name = data.get("name", "").strip()
        if oper_name:
            return oper_name  # async op handle (poll via operations.get)
        return None
    except Exception:
        return None
