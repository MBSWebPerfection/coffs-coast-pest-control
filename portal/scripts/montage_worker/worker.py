...
def process_record(record: dict[str, Any], engine: MontageEngineStatus) -> str:
    ...
    #   MONTAGE_RENDERER=openmontage -> DECOMMISSIONED — treated as FFmpeg.
    #   MONTAGE_RENDERER=auto        -> DECOMMISSIONED — treated as FFmpeg.
    # Open Montage is no longer a render engine. All values resolve to the
    # deterministic FFmpeg Ken Burns path (no node/npx, no WinError 193).
    renderer = os.environ.get("MONTAGE_RENDERER", "ffmpeg").strip().lower()
    # (c) Decommission Open Montage: keep the env var accepted for back-compat,
    # but never dispatch to it. Any historical value maps to FFmpeg-only.
    ...
    try:
        # FFmpeg-only Ken Burns. No node/npx involved, so it never hits the
        # Windows [WinError 193] .cmd-shim failure.
...
def run_on_demand(dry_run: bool = False) -> dict[str, Any]:
    ...
    #    Asset hierarchy: (a) static brand poster first (deterministic), (b)
    #    direct Google Imagen fallback ONLY when the static source is missing
    #    AND an API key is configured, (c) Open Montage decommissioned. If a
    #    poster is present we always use it (zero behavior change); if the
    #    poster is absent we request Imagen, else skip gracefully.
    for i, concept in enumerate(image_concepts, start=1):
        rid = f"on-demand-{i}"
        src = img_out_dir / f"p{((i - 1) % 6) + 1}.jpg"
        dst = img_out_dir / f"{rid}.jpg"
        try:
            if src.exists():
                # (a) static-first: re-watermark the existing brand poster.
                pipeline.watermark_image(src, dst, PORTAL_PUBLIC_DIR)
                result["images"].append({
                    "id": rid, "path": f"/images/{rid}.jpg",
                    "topic": concept["topic"], "angle": concept["angle"],
                    "caption": concept["caption"], "source": "static",
                })
                continue
            # (b) missing poster -> request Imagen (only if a key is present).
            gen_png = pipeline.generate_image_via_imagen(
                f"Square 1080x1080 social post for a pest-control business: "
                f"{concept['topic']} — {concept['caption']}"
            )
            if gen_png:
                dst.write_bytes(gen_png)
                pipeline.watermark_image(dst, dst, PORTAL_PUBLIC_DIR)
                result["images"].append({
                    "id": rid, "path": f"/images/{rid}.jpg",
                    "topic": concept["topic"], "angle": concept["angle"],
                    "caption": concept["caption"], "source": "imagen",
                })
                continue
            # (c) no static + no key/no response -> skip gracefully.
            result["errors"].append({"id": rid, "error": "no static poster and Imagen key/response unavailable"})
        except Exception as exc:  # pragma: no cover
            result["errors"].append({"id": rid, "error": str(exc)})

    # 3) Generate 3 video montages.
    #    Asset hierarchy: (a) uploaded/user media (already carries media_url and
    #    short-circuits upstream — first-class, used as-is), (b) Google Veo
    #    ONLY when USE_GOOGLE_VEO=1 AND a key is set, (c) FFmpeg Ken Burns as
    #    the zero-cost default overlay (baked logo watermark).
    ...
    for concept in video_concepts:
        ...
        try:
            out = None
            veo_src = None
            # (b) Veo is strictly opt-in (USE_GOOGLE_VEO=1 + key). Paid + async.
            if pipeline.veo_configured():
                veo_src = pipeline.generate_video_via_veo(concept["caption"])
            if veo_src:
                result["videos"].append({"id": vid, "path": veo_src, "caption": concept["caption"], "source": "veo"})
                continue
            # (c) FFmpeg default when Veo skipped/unavailable.
            out = render_via_ffmpeg_kenburns(vid, poster, rend_root / f"{vid}.mp4")
            publish_media(vid, out)
            result["videos"].append({"id": vid, "path": f"/media/{vid}.mp4", "caption": concept["caption"], "source": "ffmpeg"})
        except Exception as exc:  # pragma: no cover
